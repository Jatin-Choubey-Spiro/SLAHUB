...
contract SpiroAgreementManager {
    ...
    function createAgreement(string memory _agreementHash, string memory _ipfsCID, address[] memory _signers) public onlyOwner {
        require(_signers.length > 0, "At least one signer must be added");

        uint256 agreementId = agreementCount;
        agreements[agreementId].agreementHash = _agreementHash;
        agreements[agreementId].ipfsCID = _ipfsCID;
    ...
    function hasSignerSigned(uint256 _agreementId, address _signer) public view returns (bool) {
        Agreement storage agreement = agreements[_agreementId];
